...
The design heat transfer capacity and UA for the heat exchanger are calculated using the design temperatures for the two plant loops. The loop design temperature difference for the Loop Supply Side, \(\Delta {T_{SupLoop,Des}}\) , is entered in a Sizing:Plant object and is used to determine a nominal capacity. If the UA is not autosized, and Sizing:Plant objects are not provided, then the delta T is determined using the loop set point temperataures, or average of the loop high and low set point temperatures for DualSetpointDeadband control. Cp and rho are calculated at an initialization temperature of 5.05 C. The following equations are used to calculated Nominal Capacity where a Sizing:Plant object is specified and when loop set point temperatures are used:

\begin{equation}
\dot Q = V_{Sup,des} \rho {c_p} \Delta {T_{SupLoop,Des}}, or
\end{equation}

\begin{equation}
\dot Q = V_{Sup,des} \rho {c_p} \Delta {T_{SupLoop,Setpoint,Des - DemLoop,Setpoint,Des}}
\end{equation}
...
A reference capacity for the heat exchanger is determined from the design flow rates (regardless of if they were automatically sized or input by the user) and heat exchanger temperature difference. When UA is autosized the loop temperature difference is obtained from the inputs in Sizing:Plant objects. If UA is not autosized then the loop's setpoint temperatures are used (if the loop's load scheme is DualSetpointDeadband then the average of the high and low setpoints are used). The heat exchanger reference capacity is then calculated for the maximum loop flow rates and expected loop temperature difference. This capacity is reported and may be used for controls based on operation scheme.
