...
The Loop Demand Side design flow rate,\({\dot V_{Dmd,des}}\) , is set equal to the Loop Supply Side design flow rate multiplied by the supply to demand side ratio of the loop side specific heat (Cp) and temperature difference (Dt) product. This ratio accounts for differences in plant design delta temperatures and fluid heat capacity (e.g., water versus glycol). If Sizing:Plant objects are not present for both loops connected to the heat exchanger then this ratio is equal to 1.

\begin{equation}
{\dot V_{Dmd,des}} = {\dot V_{Sup,des} * {((Cp,sup * Dt,sup) / (Cp,dem * Dt,dem))}}
\end{equation}
