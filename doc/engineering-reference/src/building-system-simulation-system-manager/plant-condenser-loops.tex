...
\( \dot m_{br} \) = \textrm{ final~resolved~branch~flow~rate}

\( \dot m_{br\_request} \) = \textrm{ requested~branch~flow~rate }

\( \dot m_{tot\_request} \) = \textrm{ total~loop~mass~flow~rate~request}

\( \dot m_{tot\_available} \) = \textrm{ total~loop~mass~flow~rate~available}
