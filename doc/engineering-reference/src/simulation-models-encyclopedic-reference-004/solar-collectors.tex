...
\begin{equation}
{{\mathrm{b}}_{\mathrm{3}}} = {\mathrm{ }}\left( {A \cdot {U_b}{T_{osc}} + A \cdot {U_s}{T_a} + {{\dot m}_w}{C_w}{T_{wi}}} \right)
\end{equation}
...
\begin{equation}
{q_{LWR, 1}} + {q_{CONV, 1}} + {q_{solar, abs}} + {q_{LWR, 2}} + {q_{CONV, 2}} = 0
\end{equation}
...
\begin{equation}
{T_{c{\mathrm{1}}}} = \frac{{{\alpha_{c1}}{I_t} + {h_{r, c1 - a}}{T_a} + {h_{c, c1 - a}}{T_a} + {h_{r, c2 - c1}}{T_{c2}} + {h_{c, c2 - c1}}{T_{c2}}}}{{{h_{r, c1 - a}} + {h_{c, c1 - a}} + {h_{r, c2 - c1}} + {h_{c, c2 - c1}}}}
\end{equation}

\begin{equation}
{T_{c2}} = \frac{{{\alpha_{c2}}{I_t} + {h_{r, c2 - c1}}{T_{c1}} + {h_{c, c2 - c1}}{T_{c1}} + {h_{r, p - c2}}{T_p} + {h_{c, p - c2}}{T_p}}}{{{h_{r, c2 - c1}} + {h_{c, c2 - c1}} + {h_{r, p - c2}} + {h_{c, p - c2}}}}
\end{equation}
...
\begin{equation}
{q_{cond}} + {q_{conv, cav}} + {q_{rad, cav}} = 0
\end{equation}

Substituting the equations for each term in the above equation yields:

\begin{equation}
U_L \left( T_w - T_{osc} \right) + h_{c, cav} \left( T_{a, cav} - T_{osc} \right) + h_{r, cav} \left( T_{so} - T_{osc} \right) = 0
\end{equation}

Simplifying yields the bottom insulation other side condition temperature:

\begin{equation}
{T_{osc}} = \frac{{{U_L}{T_w} + {h_{c, cav}}{T_{a, cav}} + {h_{r, cav}}{T_{so}}}}{{{U_L} + {h_{c, cav}} + {h_{r, cav}}}}
\end{equation}

The cavity air temperature is determined from cavity air heat balance as follows:

\begin{equation}
{T_{a, cav}} = \frac{{{h_{c, cav}}A{T_{osc}} + {{\dot m}_{vent}}Cp{T_a} + {h_{c, cav}}A{T_{so}}}}{{{h_{c, cav}}A + {{\dot m}_{vent}}Cp + {h_{c, cav}}A}}
\end{equation}
...
\begin{equation}
Nu = 0.56{\left( {Gr \cdot \Pr  \cdot cos \theta } \right)^{1/5}}\quad {10^5} < Gr \cdot Pr < {10^{11}}
\end{equation}
...
\begin{equation}
Gr = {\frac{{g{\beta_{\mathrm{v}}}\left( {{T_p} - {T_w}} \right)L_c^3}}{{\nu ^2}}}
\end{equation}
...
\begin{equation}
N{u_a} = 1 + 1.44\left\{ {1 - \frac{{1708{{\left( {\sin 1.8\beta } \right)}^{1.6}}}}{{Ra\cos \beta }}} \right\}{\left\{ {1 - \frac{{1708}}{{Ra\cos \beta }}} \right\}} + {\left\{ {{{\left( {\frac{{Ra\cos \beta }}{{5830}}} \right)}^{{\frac{1}{3}}}} - 1} \right\}}
\end{equation}

\begin{equation}
{h_c} = {\frac{{Nu \cdot k}}{L}}
\end{equation}
...
\begin{equation}
N{u_D} = 2.75\left[ {{{\left( {\frac{P}{D}} \right)}^{ - 1.2}}\operatorname{Re}_D^{0.43} + 0.011\sigma {{\operatorname{Re} }_D}{{\left( {\frac{{{U_\infty }}}{{{V_h}}}} \right)}^{0.48}}} \right]
\end{equation}
...
\begin{equation}
\begin{split}
{\varepsilon_{HX}} =& \left[ {1 - \left( {1 + {{\operatorname{Re} }_s}Max{{\left( {1.733\operatorname{Re}_w^{ - {\frac{1}{2}}},0.02136} \right)}^{ - 1}}} \right)} \right] \times \left[ {1 - {{\left( {1 + 0.2273\operatorname{Re}_b^{{\frac{1}{2}}}} \right)}^{ - 1}}} \right] \\
&\times {e^{\left( { - 0.01895\frac{P}{D} - \frac{{20.62}}{{{{\operatorname{Re} }_D}}}\frac{t}{D}} \right)}}
\end{split}
\end{equation}
...
\({T_{{\mathrm{a,HX}}}}\) is the temperature of the air leaving the collector and entering the plenum (\(^{\circ}\)C)
...
By rewriting Equation~\ref{eq:SolarCollectorHXEffect716} to solve for \({T_{{\mathrm{a,HX}}}}\), we see that the temperature of the heated outdoor air entering the plenum can be determined once the collector surface temperature is known:
...
where \({T_{{\mathrm{a,HX}}}}\) is determined using correlations described above.~ When the UTSC is active, the air mass flow is determined from the operation of the outdoor air mixer component.~ When the UTSC is off, this term is zero.
...
\begin{equation}
{\dot m_{vent}} = \rho {{\overline{\dot V}}_{{\mathrm{tot}}}}
\end{equation}
...
\({{\overline{\dot V}}_{{\mathrm{tot}}}} = {{\overline{\dot V}}_{{\mathrm{wind}}}} + \,{{\overline{\dot V}}_{{\mathrm{thermal}}}}\) is the total volumetric flow rate of air ventilating in and out of the plenum (m\(^{3}\)/s)

\begin{equation}
{{\overline{\dot V}}_{{\mathrm{wind}}}} = {C_v}{A_{in}}{U_\infty }
\end{equation}

\begin{equation}
{{\overline{\dot V}}_{{\mathrm{thermal}}}} = {C_D}{A_{in}}\sqrt {2g\Delta {H_{NPL}}\left( {{T_{a,plen}} - {T_{amb}}} \right)/{T_{a,plen}}} ~ (if~{T_{a,plen}} > {T_{amb}} )
\end{equation}

\begin{equation}
{{\overline{\dot V}}_{{\mathrm{thermal}}}} = {C_D}{A_{in}}\sqrt {2g\Delta {H_{NPL}}\left( {{T_{amb}} - {T_{a,plen}}} \right)/{T_{amb}}} ~ (if~{T_{amb}} > {T_{a,plen}} ~ and~UTSC~vertical)
\end{equation}
...
If the UTSC is horizontal and \({T_{amb}} > {T_{a,plen}}\), then \({{\overline{\dot V}}_{{\mathrm{thermal}}}}{\mathrm{ = 0}}\) because this is a stable situation.
...
\subsubsection{Energy Balance}\label{BIPVT-energy-balance}
...
\(h_{rad,1-2}\) is the radiative heat transfer coefficient between the back of the PV modules and the building wall/roof (\(W\cdot m^{-2}K^{-1}\))
...
\subsubsection{Solving energy balance equations for zero flow condition}\label{BIPVT-solving-zero-flow}
