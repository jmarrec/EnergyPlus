...
\begin{equation}
{\mathrm{Z}_i} = ({z_{i,centroid}} - {z_{floorAvg}})/Ceiling\;Height
\end{equation}
...
The user-defined profile is treated as a look up table or piecewise linear model.~ The values for \(\Delta {T_{ai}}\) are determined by searching the \(\mathrm{Z}\) values in the user-defined profile and performing linear interpolation on the associated \(\Delta {T_a}\) values.
...
\begin{equation}
\Delta \rho /\rho  = \Delta \mathrm{T}/\mathrm{T}
\end{equation}
...
\begin{equation}
{\dot V_{\min }} = {(1.5/{\mathrm{24}}{\mathrm{.55)}}^{{\mathrm{5/3}}}} \cdot {N_{plumes}} \cdot \dot Q_{perplume}^{1/3}
\end{equation}
...
If \({T_{mx}} < {T_{oc}}\) ~or \(MC{P_{tot}} \le 0\) ~or \({H_{fr}} \cdot {H_{ceil}} < {H_{fl,top}} + \Delta {z_{occ,\min }}\) ~or \({T_{mx}} - {T_{oc}} < \Delta {T_{Crit\operatorname{Re} p}}\)
