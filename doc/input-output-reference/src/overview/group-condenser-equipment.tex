...
\begin{enumerate}
    ...
    \item Use a third-party tool such as GLHEPro to compute the g-functions, or download them from a \href{https://gdr.openei.org/submissions/1325}{precomputed library} and pass them to EnergyPlus using the \hyperref[groundheatexchangerresponsefactors]{GroundHeatExchanger:ResponseFactors} object.
    \item Request borehole field design and sizing by providing a single \hyperref[groundheatexchangerverticalsingle]{GroundHeatExchanger:Vertical:Single} object and also a \hyperref[groundheatexchangerverticalsizingrect]{GroundHeatExchanger:Vertical:Sizing:Rectangle} object.
\end{enumerate}
...
\paragraph{Field: GHEVertical:Sizing object type}

For borehole field design and sizing calculations, this should be the type of an object specifying the borehole field design parameters.
Currently only GroundHeatExchanger:Vertical:Sizing:Rectangle is supported.

\paragraph{Field: GHE:Vertical:Sizing object name}

For borehole field design and sizing calculations, this should be the name of an object specifying the borehole field design parameters.
See the full documentation for \hyperref[groundheatexchangerverticalsizingrect]{GroundHeatExchanger:Vertical:Sizing:Rectangle}.
...
The unique names of all single, vertical GLHE objects which are used to define and arbitrarily shaped GHE array.
This field is extensible.

The following are example inputs corresponding to the input variations allowed which were described previously.
The first example uses the \hyperref[groundheatexchangerverticalarray]{GroundHeatExchanger:Vertical:Array} object.
The g-functions will be generated automatically by EnergyPlus.
...
The second example uses the \hyperref[groundheatexchangerverticalsingle]{GroundHeatExchanger:Vertical:Single} object.
The g-functions will be generated automatically by EnergyPlus.
...
\subsection{GroundHeatExchanger:Vertical:Sizing:Rectangle}\label{groundheatexchangerverticalsizingrect}
...
HVAC sizing must also be enabled.

\subsubsection{Inputs}

\paragraph{Field: Name}

This alpha field contains the identifying name for the rectangle ground heat exchanger sizing object.
This name will be referenced in a GroundHeatExchanger:System object.

\paragraph{Field: SizingPeriod:WeatherFileDays Name}

The name of the SizingPeriod:WeatherFileDays object, used to define the design period.
This object is required and must specify a period of 356 days.

\paragraph{Field: Design Flow Rate Per Borehole}

This is the design flow rate per borehole in m3/s.
Used only for sizing the ground heat exchanger.
...
GroundHeatExchanger:Vertical:Sizing:Rectangle,
  GHE-1-Sizing,               !- Name
  GHE Sizing Period,          !- SizingPeriod:WeatherFileDays Name
  0.0005,                     !- Design Flow Rate Per Borehole
