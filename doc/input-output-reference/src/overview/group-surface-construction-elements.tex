...
In the example above, because the inside face absorptance values are not specified separately, those values will default to the respective outside face absorptance values entered by the user.
...
In this second example, different values have been assigned to both the inside and outside faces for the various absorptance values.
...
The thermal absorptance field in the Material input syntax represents the fraction of incident long wavelength (>2.5 $\mu{}m$) radiation that is absorbed by the material. This particular field is for the exterior side or outside face. This parameter is used when calculating the long wavelength radiant exchange between various surfaces and affects the surface heat balances. For long wavelength radiant exchange, thermal emissivity and thermal emittance are equal to thermal absorptance. Values for this field must be between 0.0 and 1.0 (with 1.0 representing ``black body'' conditions). The default value for this field is 0.9. This field will also set the thermal absorptance at the inside face or interior side of the material when the optional Thermal Absorptance Inside Face field below is not set.
...
with EMS. This object adjusts the thermal or solar absorptance of the
outside and/or inside face of a material. Another opportunity to adjust these values is also available through the use of EMS.
...
The name of a Schedule object that overwrites the material thermal absorptance at the outside face or exterior side. If neither this field or the previous field are defined, then the thermal absorptance is assumed to be constant.
...
The name of a Schedule object that overwrites the material solar absorptance at the outside face or exterior side. If neither this field or the previous field are defined, then the solar absorptance is assumed to be constant.

\paragraph{Field: Control Signal Inside Face}\label{field-control-signal-inside-face}

For the inside face or interior side of the material, the control signal can be one of the following: surface temperature, surface received solar radiation, zone heating/cooling mode, or a schedule. If the control signal is "Scheduled", then a schedule needs to be specified in "Thermal Absorptance Schedule Name Inside Face" or "Solar Absorptance Schedule Name Inside Face". The schedule value will override the material absorptance value. If the control signal is not "Scheduled", then the control signal value at the target surface or zone will decide the absorptance, based on the function referenced in "Thermal Absorptance Function Name Inside Face" or "Solar Absorptance Function Name Inside Face". If this field is not specified, the inside-face control signal and its associated functions or schedules default to the outside-face settings.
...
The name of a Schedule object that overwrites the material thermal absorptance at the inside face or interior side. If neither this field or the previous field are defined, then the thermal absorptance is assumed to be constant.
...
The name of a Schedule object that overwrites the material solar absorptance at the inside face or interior side. If neither this field or the previous field are defined, then the solar absorptance is assumed to be constant.
...
\begin{lstlisting}
  MaterialProperty:VariableAbsorptance,
    ...
    SurfaceTemperature,      !- Outside Face Control Signal
    ThermalAbsTempTable,     !- Outside Face Thermal Absorptance Function Name
    ,                        !- Outside Face Thermal Absorptance Schedule Name
    SolarAbsTempCurve,       !- Outside Face Solar Absorptance Function Name
    ;                        !- Outside Face Solar Absorptance Schedule Name
\end{lstlisting}

In the example above, because the inside face absorptance variations are not specified separately, those values will default to the respective outside face absorptance variations entered by the user.
