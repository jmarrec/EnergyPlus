...
For backward compatibility, the existing Thermal, Solar, and Visible Absorptance fields retain their bulk-material meaning. If the corresponding optional Inside Face field is blank, the existing value is used for both surface faces. If an Inside Face value is entered, the existing field supplies the outside-face value and the new field supplies the inside-face value. For an opaque construction, only the outside-face properties of the outermost layer and the inside-face properties of the innermost layer affect the surface heat balances; absorptance values on intermediate layers are not surface properties.
