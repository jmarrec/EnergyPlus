...
The People statement name listed here must match the actual name provided in the People statement referenced by this field. For People objects that reference a specific zone description without spaces or directly reference a space, this is simply the name of a valid People object in the input file. However, in the case where a People statement is defined for a zone that includes multiple spaces, EnergyPlus internally expands the People statement and alters the People name. For example, if a People statement is named Zone X People, is defined in Zone X, and Zone X contains two spaces called Space X1 and Space X2, the People statement internally is expanded to create two new People objects called "Space X1 Zone X People" and "Space X2 Zone X People" for Space X1 and Space X2, respectively. These expanded names are the names that can be referenced in this input field. "Zone X People" is no longer a valid name on its own due to this expansion for spaces.
...
where each of these People names are different People definitions that are all assigned to the zone named West Wing Zone that does not have any spaces associated with it.

For the case where West Wing Zone is a zone with a single People object named West Wing People associated with it but contains spaces named Oval Office, Staff Office, and Conference Room, the following object would be valid:
