...
EnergyPlus has the ability to analyze various MRT calculations via the \hyperref[people]{People} statement. In the People input object, the user can select to have the standard MRT calculation impact the thermal comfort model or either a surface weighted or angle factor calculation (see \hyperref[field-mean-radiant-temperature-calculation-type]{Mean Radiant Temperature Calculation Type}) define the MRT for the thermal comfort model(s) chosen. Since it is possible to have multiple People statements per zone and each one can have a different MRT calculation, it might be desirable to have the zone MRT be based on one or more of these People statement defined MRT values. This would impact both the MRT of the zone as well as the operative temperature of the zone and thus would affect how systems that are controlled on either one of these temperatures to function. It is important to note that any of the People objects referenced in the ZoneMRTCalculation input object must select a thermal comfort model in order for the specific MRT calculation type in that People object to activate the calculation type.  Moreover, only a single ZoneMRTCalculation object can be linked to each unique zone.

The ZoneMRTCalculation input object defines the relative weighting or contribution each people-based MRT contributes to the calculated zone MRT. It contains a zone name that refers to the zone in question and then continues with pairs of parameters that reference a specific people statement and the relative weighting factor that each of these statemnts contributes to the zone MRT. This list is extensible and can be as short as a single pair. All people objects referenced in this extensible list must be in the same zone.

The sum of all weighting factors entered through a single ZoneMRTCalculation instance must be between 0.0 and 1.0. Error messages will be produced when this principle is violated. If the sum of all of the weighting factors is less than 1.0, the remaining portion will be based on the standard zone MRT that is an air-emissivity weighted average of all of the surface temperatures in the zone. A warning message is produced to note the user that the sum of all the weighting factors is less thann 1.0 simply to alert the user in case this was not the intent. The equation for this calculation is as follows:
...
This field specifies the name of the People statement that will contribute to the zone MRT calculation. In conjunction with the next parameter, it defines a fraction of the MRT calculation value for the zone named in the previous input. Each People statement referenced must be used in the zone referenced in the Zone Name field above.

The People statement name listed here must match the actual name provided in the People statement referenced by this field. For People objects that reference a specific zone description without spaces or directly reference a space, this is simple the name of a valid People object in the input file. However, in the case where a People statement is defined for a zone that includes multiple spaces, EnergyPlus internally expands the People statement and alters the People name. For example, if a People statement is named Zone X People, is defined in Zone X, and Zone X contains two spaces called Space X1 and Space X2, the People statement internally is expanded to create two new People objects called "Space X1 Zone X People" and "Space X2 Zone X People" for Space X1 and Space X2, respectively. These are then valid names that can be referenced in this input field. "Zone X People" is no longer a valid name on its own due to this expansion for spaces.  Note also that if Zone X is not fully defined by Space X1 and Space X2 that there is a "remainder". This portion of the zone would also receive a new people object with the name "Zone X-Remainder" combined with the original name of "Zone X People" to create the name for a new people object of "Zone X-Remainder Zone X People". This is then also a valid name and can be referenced in this People Statement Name field. See the IDF examples below for more information.

In addition, it is important to note that any People object referenced here MUST include at least one \hyperref[field-thermal-comfort-model-type-up-to-7-allowed]{Thermal Comfort model} selection in the People object.
...
where each of these people are different People defintions that are all assigned to the zone named West Wing Zone that does not have any spaces associated with it.

For the case where West Wing Zone is a zone with a single People object named West Wing People associated with it but contains spaces named Oval Office, Staff Office, and Conference Room, the following, the following object would be valid:
...
ZoneMRTCalculation,
  ...
  0.3,
  West Wing Zone-Remainder West Wing People, !- People 4 Name
  0.4;
\end{lstlisting}
